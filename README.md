...
2. **Config**
   ...
   - `transform_output_dir` — where to write the transformed file before upload; if omitted or empty, `./output` (created if needed). The local file is deleted after the SFTP upload attempt (success or failure). The file basename is always **`chargebee_journal_posting.csv`** (see `TRANSFORM_OUTPUT_FILENAME` in `const.py`); repeated runs overwrite the same file.
   - **AccountingDate** is the journal **`Transaction Date`** normalized to **`YYYY-MM-DD`** when the cell parses (supports `YYYY-MM-DD`, `YYYY/MM/DD`, `MM/DD/YYYY`, `DD/MM/YYYY`, hyphen variants, and ISO datetimes — date part only). Blank / `\N`-like → empty; unparseable values are left as the trimmed cell string.
   - `config_fields` — JSON **object** or **string** of an object: keys merged onto the flat config (usually Workday journal CSV header names and related defaults). Applied before top-level keys (so top-level wins). Empty strings are ignored. Nothing in `const.py` validates the key set—transform reads what it needs from the merged flat dict. Row-driven columns (`Currency`, `LedgerAccountReferenceID`, `LineMemo`, revenue/sales worktags, amounts, etc.) come **only** from the journal row (trimmed); blank cells stay blank in the output. **`LineCompanyReferenceID`** is the journal **`MarketID Finance`** cell with suffix **`_400`** when that cell is non-blank (e.g. `ACME` → `ACME_400`); otherwise it uses **`LineCompanyReferenceID`** from the flattened config (if set). Other Workday columns are set from the flattened config via `_str_from_config` (same idea as Oracle for config-only GL fields). Top-level-only settings (`journal_line_memo_prefix`, transform paths, SFTP keys, etc.) stay outside this blob.
